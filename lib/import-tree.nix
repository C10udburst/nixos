let
  perform = {
    ...
    in {
      imports =
        if scoped == {}
        then files
        else map scoped-import-module files;
    };
  ...
  removePrefix = pre: s: let
    n = builtins.stringLength pre;
  in
    if builtins.substring 0 n s == pre
    then builtins.substring n (builtins.stringLength s - n) s
    else s;
  ...
  hasInfix = infix: content: let
    lenInfix = builtins.stringLength infix;
    lenContent = builtins.stringLength content;
    go = i:
      if i + lenInfix > lenContent
      then false
      else if builtins.substring i lenInfix content == infix
      then true
      else go (i + 1);
  in
    go 0;
  ...
  listDirFilesRecursive = dir: let
    entries = builtins.readDir dir;
  in
    builtins.concatMap (
      name:
        if entries.${name} == "directory"
        then listDirFilesRecursive (dir + "/${name}")
        else [(dir + "/${name}")]
  ...
  functor = self: arg:
    if builtins.isFunction arg && builtins.functionArgs arg == {}
    then arg self # arg is a combinator pass the self import-tree obj
    else if inModuleEval arg
    then perform self.__config [] arg
    else perform self.__config arg;
