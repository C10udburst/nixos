...
with lib; let
  ...
  # widget logic from original config
  wGap = 6;
  ws = {
    clock = {
      w = 380;
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."driftwm/catppuccin_marble.glsl".source = ./driftwm_assets/catppuccin_marble.glsl;
    xdg.configFile."driftwm/plasma.glsl".source = ./driftwm_assets/plasma.glsl;

    xdg.configFile."driftwm/config.toml".text = ''
      mod_key = "super"
      focus_follows_mouse = true
      window_placement = "auto"
      autostart = ["widget-launcher"]
