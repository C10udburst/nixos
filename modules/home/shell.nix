...
in {
  ...
  config = lib.mkMerge [
    {
      # Wrap ssh so the remote host sees TERM=xterm instead of xterm-kitty
      # (most servers lack xterm-kitty in their terminfo database).
      # scp/sftp/rsync invoke the ssh binary directly and are unaffected.
