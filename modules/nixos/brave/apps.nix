...
let
  ...
  webApp =
    ...
    let
      ...
      windowSizeArg = if size != "" then " --window-size=${size}" else "";
    in
    pkgs.makeDesktopItem {
      name = "webapp-${sanitizedName}";
      desktopName = name;
      exec = "brave --app=${url}${windowSizeArg}";
      icon = if icon != "" then icon else "brave-browser";
      terminal = false;
      type = "Application";
      inherit categories;
      comment = if comment != "" then comment else "${name} Web Application";
    };
  ...
  isVscodeEnabled =
    if builtins.isAttrs (config.hostSettings.vscode or true) then
      config.hostSettings.vscode.enable or true
    else
      config.hostSettings.vscode or true;

  isSocialEnabled =
    if builtins.isAttrs (config.hostSettings.social or false) then
      config.hostSettings.social.enable or false
    else
      config.hostSettings.social or false;
  ...
  convertUserWebApp =
    ...
    if lib.isDerivation item then
      ...
      throw "Unsupported webapp entry: ${builtins.toJSON item}";
